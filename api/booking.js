...
<div style="font-family: 'PingFang SC', 'Microsoft YaHei', sans-serif; max-width: 700px; margin: 0 auto; color: #333333; line-height: 1.8; font-size: 15px; background-color: #ffffff; padding: 20px;">
    ...
    <h3 style="font-size: 16px; font-weight: bold; margin-top: 25px; border-bottom: 1px dashed #cccccc; padding-bottom: 8px;">【锁定凭证】</h3>
    ...
    <h3 style="font-size: 16px; font-weight: bold; margin-top: 30px; border-bottom: 1px dashed #cccccc; padding-bottom: 8px;">【一、核心诉求】</h3>
    <p style="margin: 8px 0;"><strong>本次推演聚焦：</strong> ${caseReport.coreFocus || '未选定'}</p>
    
    <h3 style="font-size: 16px; font-weight: bold; margin-top: 30px; border-bottom: 1px dashed #cccccc; padding-bottom: 8px;">【二、时空结构】</h3>
    ...
    <h3 style="font-size: 16px; font-weight: bold; margin-top: 30px; border-bottom: 1px dashed #cccccc; padding-bottom: 8px;">【三、事业学业】</h3>
    <p style="margin: 8px 0;"><strong>行业职向：</strong> ${ce.career || '未填'}</p>
    <p style="margin: 8px 0;"><strong>子女学业：</strong> ${ce.edu || '未填'}</p>

    <h3 style="font-size: 16px; font-weight: bold; margin-top: 30px; border-bottom: 1px dashed #cccccc; padding-bottom: 8px;">【四、五伦关系】</h3>
    <p style="margin: 8px 0;"><strong>夫妇有别 (伴侣)：</strong> ${fr.spouse || '未明'}</p>
    <p style="margin: 5px 0;"><strong>父子有亲 (亲子)：</strong> ${fr.parentchild || '未明'}</p>
    <p style="margin: 5px 0;"><strong>君臣有义 (尊卑)：</strong> ${fr.workplace || '未明'}</p>
    <p style="margin: 5px 0;"><strong>长幼有序 (长幼)：</strong> ${fr.originfamily || '未明'}</p>
    <p style="margin: 5px 0;"><strong>朋友有信 (社交)：</strong> ${fr.social || '未明'}</p>

    <h3 style="font-size: 16px; font-weight: bold; margin-top: 30px; border-bottom: 1px dashed #cccccc; padding-bottom: 8px;">【五、其他补充】</h3>
    ...
    <div style="margin-top: 50px; padding-top: 15px; border-top: 1px solid #dddddd; text-align: center; font-size: 12px; color: #999999;">
        此案卷由河洛咨询前置对齐系统自动汇总生成
                        ...
                        if (process.env.PUSHPLUS_TOKEN) {
                            ...
                            }).catch(err => console.error("微信推送插件异常，但不影响主逻辑:", err));
                        }

                    } catch (emailError) {
                        console.error("邮件投递干扰:", emailError);
                    }
                }  
